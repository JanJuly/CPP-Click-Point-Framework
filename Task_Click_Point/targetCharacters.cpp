...
/*void targetCharacters::update()
{

}

void targetCharacters::updateMove()
{
	...
	if (isMoving)
	{
		...
		if (sprite.getPosition().x > mousePosition.x)
		{
			...
			sprite.setTextureRect(sf::IntRect(walkingCounterPersistent * 32, 32 * 1, 32, 32)); //Loop through sprite sheet for animation
		...
		{
			sprite.move(moveSpeed, 0); //Move in +X direction
			sprite.setTextureRect(sf::IntRect(walkingCounterPersistent * 32, 32 * 2, 32, 32));
